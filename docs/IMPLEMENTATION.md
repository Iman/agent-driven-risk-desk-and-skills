...
- Eight root documents added: CLA.md, CODE_OF_CONDUCT.md,
  CONTRIBUTING.md, CONTRIBUTORS.md, FAQ.md, INSTALL.md, LICENSES.md and
  PRIVACY.md. Adapted from the sibling project rather than copied; every
  options-specific sentence was removed and a validation test now fails
  the build if one returns.
- Two were deliberately NOT created, and an empty shell of either would
  have been worse than its absence. GEMINI.md there is generated by a
  tool from that project's SKILL.md files; this repository has no such
  generator, so a hand-written copy of five skills would drift from them
...
- 10 validation tests check the documents against the repository: every
  ...
  document claims this software fetches market data, every `riskdesk`
  command a document names exists in the CLI, the licence is identical in
  LICENSE, LICENSES.md, pyproject, four documents and both plugin
  manifests, and the notice counts in LICENSES.md and THIRD-PARTY.md
  agree with the directory on disk.
- The four dependency licences LICENSES.md names were each checked
  against the notice file that ships, not against a package index:
  skfolio BSD 3-Clause, MCP Python SDK MIT, Pydantic MIT, and ORE's
  Quaternion Risk Management and Acadia notice. The other 43 are
  deliberately not named with a licence, and LICENSES.md says why: 16 of
  the 48 inventory entries declare no licence expression at all.
- Three of those tests failed on their first run and each failure was the
  test guessing rather than checking. One flagged FAQ.md for the words
  "Greeks" and "volatility surface" in a sentence DENYING them, and is now
...
- Claims verified against the code before being written down: 33
  ...
  `--host 10.0.0.1`; and `riskdesk-mcp` on empty stdin exits 0 with no
  traceback, which is the diagnostic INSTALL.md ends with.
...
- Re-measured after the frame changed the rendered pages:
  ...
  spare.
- 268 tests collected then, all passing and none skipped, up from 267. Unit
