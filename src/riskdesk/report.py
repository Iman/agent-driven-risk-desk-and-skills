"""A self-contained HTML page built from a stress result and a tail result.
...
def report_payload(stress_result, tail_result=None, tail_pnl=None):
    """Shape the two envelopes into everything the page prints.

    The stress result carries the provenance for the page. A tail result is
    optional; when it is absent the page says so rather than leaving a gap.
    A tail result in another currency is refused, because the page prints
    one currency in its header and would otherwise mislabel the chart.
...
def render_html(payload):
    ...
    out = ["<!DOCTYPE html>", '<html lang="en"><head><meta charset="utf-8">',
           ...
           'initial-scale=1">',
           "<title>Risk Desk stress and tail report</title>",
           "<style>" + STYLE + "</style></head><body>"]
    out.append("<h1>Risk Desk stress and tail report</h1>")
...
def build_report(stress_payload, tail_payload=None):
    """Run the two analytics and return (html, payload)."""
